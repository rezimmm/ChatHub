    ...
    for (const file of files) {
      ...
      if (result) {
        ...
        const content = isImage 
          ? `[Image: ${result.file_name}](${BACKEND_URL}${result.file_url})`
          : `[File: ${result.file_name}](${BACKEND_URL}${result.file_url})`;
                  ...
                  <div className={`flex gap-2 sm:gap-3 hover:bg-gray-50 dark:hover:bg-slate-800/50 -mx-2 px-2 ${isConsecutive ? 'py-0.5' : 'py-2'} rounded-lg transition-colors`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isFileMessage ? (
                        <div className="mt-1">
                          {isImageFile ? (
                            <a href={fileMatch[3]} target="_blank" rel="noopener noreferrer" className="block max-w-sm">
                              <img src={fileMatch[3]} alt={fileMatch[2]} className="rounded-lg max-h-64 object-cover border border-gray-200 dark:border-slate-700 hover:opacity-90 transition-opacity" data-testid="message-image" />
                              <span className="text-xs text-gray-500 mt-1 block">{fileMatch[2]}</span>
                            </a>
                          ) : (
                            <a href={fileMatch[3]} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors max-w-sm" data-testid="message-file">
