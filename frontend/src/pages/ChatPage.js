...
export default function ChatPage({ user, token, onLogout }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      await fetchUsers();
      if (channelData.length > 0) setCurrentChannel(channelData[0]);
      setLoading(false);
    };
    init();
    connectWebSocket();
  ...
  const createDirectMessage = async (targetUserId) => {
    ...
    try {
      ...
      if (r.ok) { const nc = await r.json(); setChannels(prev => [...prev, nc]); setCurrentChannel(nc); setUserListOpen(false); }
  ...
  const updateUserProfile = async (updates) => {
    try {
      ...
      if (r.ok) { setCurrentUser(await r.json()); toast.success('Profile updated!'); return true; }
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      return await r.json();
    } catch (error) { toast.error(error.message || 'Failed to upload file'); return null; }
  };
